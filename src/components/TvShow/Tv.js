...
function Tv() {
    ...
    useEffect(() => {
        async function fetchData() {
            const response = await axios.get(`https://api.themoviedb.org/3/tv/top_rated?api_key=7f46651666f1ca68e4cf0cb150551f07&language=en-IN&with_original_language=hi`)
                        ...
                        {data && data.map((slide, index) => (
                            ...
                            >
                                ...
                                <div className="carousel-caption  d-md-block ">
                                    <h2 className='text-light'>{slide.original_name}</h2>
                                    <p>{slide.overview.slice(0, 180)}...</p>
                                    <h5> Language : {slide.original_language}ndi</h5>
                                    <h5>Release Date : {slide.first_air_date}</h5>
                                </div>
                            </div>
                        ))}
                    </div>
